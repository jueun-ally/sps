...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    # softmax loss
    loss += torch.sum(-torch.log(prob[torch.arange(N), y]))
    loss /= N
    ...
    # score = h1 * W2 + b2
    grads['W2'] = torch.matmul(h1.t(), dscore)
    grads['b2'] = torch.sum(dcsore, dim=0)
    dh1 = torch.matmul(dscore, W2.t())
    
    # h1 = max(0, z)
    dz = dh1 * (h1 > 0)
    
    # z = X * W1 + b1
    grads['W1'] = torch.matmul(X.t(), dz)
    grads['b1'] = torch.sum(dz, dim=0)
...
def nn_train(params, loss_func, pred_func, X, y, X_val, y_val,
  ...
  for it in range(num_iters):
    ...
    #########################################################################
    # Replace "pass" statement with your code
    random_idx = torch.randint(num_train, (batch_size,))
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  scores, _ = nn_loss_part1(params, X, y=None, reg=0.0)
